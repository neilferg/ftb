...
class TreeBuilder:
    ...
    def makeHtml(self, title, dotFile):
        ...
        with open(htmlFilePath, "r") as fs: # generated by graphviz
            text = fs.read()
        origMapDoc = lxml.html.fromstring(text)

        with open(self.templ, "r") as fs: # our template
            text = fs.read()
        ...
        if True: # base64 encoded
            with open(imageFilePath, 'rb') as fs:
                img = fs.read()
            img = b"data:image/png;base64,"+base64.b64encode(img)  
            bitmap.attrib["src"] = img.decode("utf8")
